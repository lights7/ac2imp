...
#        if 'Customer'|'Employee' in grid.grid_contents[row][2]: 
        ...
        a=['Customer','Employee','Vendor','Personal']
...
#        if 'Customer' in grid.grid_contents[row][2] or 'Employee' in grid.grid_contents[row][2] not grid.grid_contents[row][1].find("None"): 
            ...
            acct['TODAY'] = today
            acct['FileName']=donor_id + donor_name.replace(' ','_') + '.xlsx'
#        uacct="%s-%s" % (mapping['BANKID'](row,grid), mapping['ACCTID'](row,grid))
#         print("ColNum=",grid.GetColNum(row))
#        print("export2")
...
#               cell.format.set_top()
           ...
           for tran in acct['trans']:
               ...
               I=I+1
               print("Amount Received=",tran['Amount Received'])
               if acct['ID'][0]=="D":
                   amtd=amtd+float(tran['Amount Received'][1:].replace(',',''))
               if acct['ID'][0]=="J":
                   amtj=amtj+float(tran['Amount Received'][1:].replace(',',''))

           worksheet.merge_range("D%d:E%d"%(I+1,I+1), 'Deductable Amount:', format_bc)
           worksheet.write(I,5,acct['Total'],format_c)
           if acct['ID'][0]=="D":
               tot=float(acct['Total'][1:])
               totald=totald+tot
#               print("totd=",tot, totald)
               print("totd=",tot, totald, acct['Name'])

           if acct['ID'][0]=="J":
               tot=float(acct['Total'][1:])
               totalj=totalj+tot
               print("totj=",tot, totalj, acct['Name'])

           worksheet.merge_range("A%d:F%d"%(I+2,I+2), " ", format_bot_line)

           if re.search('zhao', tran['Memo'], re.IGNORECASE):
              worksheet.write(I+2,0,"If you would like to receive update about Jian An & Laura Zhao's mission more frequently, please add ")
              worksheet.write(I+3,0,'our WeChat id zhao13008531569')

           worksheet.write(I+5,0,'Thank you for your gift to us for the purpose of world missions! No goods or services were received for')
           worksheet.write(I+6,0,'the above contribution. Your Love for and confidence in us is greatly appreciated. We cannot adequately')
           worksheet.write(I+7,0,'express our many thanks for your prayers and financial support.')
